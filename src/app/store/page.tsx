...
export default function StorePage() {
  ...
  const isSearching = Boolean(filters.searchQuery?.trim());
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="space-y-1">
              <p className="text-sm text-muted-foreground">
                {isSearching
                  ? t("catalog.searchResultsFor", { query: filters.searchQuery })
                  : t("catalog.productsFound", { count: filtered.length })}
              </p>
              {isSearching && (
                <p className="text-xs text-muted-foreground">
                  {filtered.length > 0
                    ? t("catalog.showingMatchingProducts")
                    : t("catalog.noMatchingProducts")}
                </p>
              )}
            </div>
